...
create policy "Published papers are public" on public.papers for select using (status = 'Published' or exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'));
create policy "Published paper questions are public" on public.questions for select using (exists (select 1 from public.papers where papers.id = paper_id and (papers.status = 'Published' or exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'))));
create policy "Published paper options are public" on public.options for select using (exists (select 1 from public.questions join public.papers on papers.id = questions.paper_id where questions.id = question_id and (papers.status = 'Published' or exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'))));
