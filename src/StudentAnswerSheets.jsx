...
export default function StudentAnswerSheets({ store, setReviewAttempt, onChangePassword, onDeleteStudent, onDeleteAttempt, onSaveComment }) {
  ...
  const [commentAttempt, setCommentAttempt] = useState(null);
  ...
  const submitComment = async event => {
    event.preventDefault();
    if (commentText.length > 500) return setCommentError('Use 500 characters or fewer.');
    setSavingComment(true); setCommentError('');
    try { await onSaveComment(commentAttempt.user, commentAttempt.attempt, commentText.trim()); setCommentAttempt(null); } catch (error) { setCommentError(error.message || 'Comment could not be saved.'); } finally { setSavingComment(false); }
  };
  ...
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">Classroom directory</p><h1>Student answer sheets</h1><p className="muted">Every paper attempt is listed separately so you can review all submitted answers.</p></div></div>
    ...
    <div className="table-wrap"><table><thead><tr><th>Student</th><th>Paper</th><th>Date</th><th>Score</th><th>Actions</th></tr></thead><tbody>{filteredRows.length ? filteredRows.map(({ user, attempt, paper }) => <tr key={attempt.id}><td><div className="student-cell"><div className="avatar small-avatar">{initials(user.name)}</div><strong>{user.name}</strong></div><small>{user.course} · {user.year}</small></td><td><strong>{attempt.title}</strong><small>{paper ? `${paper.questions.length} questions` : 'Paper deleted'}</small>{attempt.comment && <small className="result-comment">Comment: {attempt.comment}</small>}</td><td>{new Date(attempt.completedAt).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}</td><td><strong className={attempt.score >= 70 ? 'good-score' : 'low-score'}>{attempt.score}%</strong><small>{attempt.correct}/{attempt.total} correct</small></td><td><div className="result-actions">{paper ? <button className="review-button" onClick={() => setReviewAttempt({ paper, attempt })}>View sheet <Eye size={14} /></button> : <span className="muted">Unavailable</span>}<button className="review-button" onClick={() => { setCommentAttempt({ user, attempt }); setCommentText(attempt.comment || ''); setCommentError(''); }}>Comment</button><button className="review-button" onClick={() => { setPasswordStudent(user); setNewPassword(''); setPasswordError(''); }}><KeyRound size={14} /> Password</button><button className="review-button danger-text" onClick={() => onDeleteAttempt(user, attempt)}><Trash2 size={14} /> Delete</button></div>{commentAttempt?.attempt.id === attempt.id && <form className="comment-form" onSubmit={submitComment}><textarea value={commentText} onChange={event => setCommentText(event.target.value)} placeholder="e.g. Good work, or try again." maxLength="500" autoFocus /><div><button className="secondary-button" type="button" onClick={() => setCommentAttempt(null)}>Cancel</button><button className="primary-button" type="submit" disabled={savingComment}>{savingComment ? 'Saving...' : 'Share comment'}</button></div>{commentError && <small className="form-error">{commentError}</small>}</form>}{passwordStudent?.id === user.id && <form className="password-form" onSubmit={submitPassword}><input type="password" value={newPassword} onChange={event => setNewPassword(event.target.value)} placeholder="New password" autoFocus /><button className="primary-button" type="submit" disabled={savingPassword}>{savingPassword ? 'Saving...' : 'Save'}</button>{passwordError && <small className="form-error">{passwordError}</small>}</form>}</td></tr>) : <tr><td colSpan="5"><div className="empty-state"><p>No answer sheets match these filters.</p></div></td></tr>}</tbody></table></div>
